...
**关键交付**
- [ ] `PlatformContext`（聚合：Tenant / User / Workspace / Locale / Theme）—— 增量建设，先 `TenantContext`
- [ ] `Tenant`（`src/Domain/Organization/Tenant.cs`，已存在）扩展 `TenantSetting / TenantLocale / TenantTheme / TenantWorkspace / TenantUser / TenantRole / TenantPermission`
- [ ] 所有核心 Runtime 入口改为接受 `PlatformContext` 而非仅 `Question`（先 Tenant 注入，逐步扩）
- [ ] 持久化层全量加 `TenantId` 过滤；`SuperBIContext` 查询自动带租户作用域
- [ ] 新增 `TenantManagementController`（生产）+ Migration
...
**关键交付**
- [ ] `Localization` 资源体系（zh-CN/zh-TW/en-US/ja-JP/ko-KR…）
- [ ] 业务语义多语言标签映射（如 销售额/Sales Amount/売上高/매출액 映射到同一 Semantic Concept）
- [ ] `LocaleContext` 接入 `PlatformContext`；AI 意图理解语言无关化
- [ ] 问答/错误/消息多语言
