...
> 2026-09-14 浏览器补充验证：内置浏览器曾出现表单提交 username 必填，独立 Chromium 使用真实表单登录未复现；AuthFlowTests 3/3 通过，并新增整页刷新后 Ask 输入框可见的回归断言。最终登录+权限矩阵 20 项：19 通过、0 失败、1 跳过（无智能体种子，管理员 Run 正向未覆盖）。内置浏览器输入兼容性仍待查，不据此认定产品登录缺陷；业务链与 G1 仍未验收。详见 [浏览器预验证](ops/browser-preflight-20260914.md)。
...
| CI-02 | M13-05 | CI 执行全部 Unit Tests | DONE | M13-05 | **2026-09-13 完成**（CI run `34746710044` 四 job 全绿，单测 **1168/1168**）。此前 `tests/SuperBuilder_AI.Tests`（136 文件 / 1061 个 `[Fact]/[Theory]`）**不在 `SuperBuilder_AI.CI.slnf` 中**，从未在 CI 编译或执行。落地要点：① `unit-tests` job 让测试工程「自带完整 restore+build」且**不**携带全局 `-p:TargetFrameworks`（否则污染 NuGet 资产关联 → xunit 适配器不进输出 → `No test is available` → 0 用例假绿）；② `SuperBuilder_AI.Components` 在 `CI=true` 下只产 `net10.0`（规避 android/ios workload）；③ 执行数取 trx `<Counters total>`（非 console 文本），`total=0` 即 `::error::` + `exit 1`。**首次执行即暴露 4 个编译错误 + 3 个测试失败，全部修复**（`abb7613`/`29c49a3`/`249510b`/`7f62ff4`，含 OBS-01 告警重复发的真实实现缺陷） |
...
| DR-01 | M13-10 | SQL + Qdrant + config/secret reference 完整恢复（**2026-09-13 真实数据级演练闭环**：SQL 备份 2.3s / 还原 0.124s 全部核心表行数一致 + Qdrant 592MB 快照还原 109s / 1061 点一致 + 空库重放 46 迁移 / 52 表；新增并验证原生 T-SQL `backup-db-native.sql`/`restore-db-native.sql`，修复 DR 清单脚本路径与陈旧迁移数。**残留**：RPO/RTO 目标值待 OPEN「性能与恢复目标」定稿） | DONE | M13 |
...
| CACHE-01 | M13-16 | 撤权、重启、双实例缓存一致性 | DONE | M13 |
...
| ID | 工作项 | 状态 | Milestone |
|---|---|---|---|
| M14-01 | 试点产品包与验收数据集（2026-09-14 候选问题及证据规则见 product/pilot-package.md；O1/O3 与真实验收未完成） | ACTIVE | M14 |
...
| M14-07 | 支持与客户交接包（**2026-09-13 草稿已交付**：`docs/handover/` 7 册 = 安装升级/配置清单/运维手册/故障支持/备份恢复/数据退出/验收自检；服务目标与 RPO/RTO 为占位，待 PERF-01 与 OPEN 回填后定稿。**G1 只接受定稿版**） | ACTIVE | M14（G1 验收前须就绪） |
| M14-08a | 试点成效测量方案（2026-09-14 草案见 product/pilot-measurement.md；阈值、周期及采集演练未完成） | ACTIVE | M14（G1 前） |
