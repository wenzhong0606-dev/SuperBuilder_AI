...
| BI-01 | CI/M13-06 | GQ-007 多指标 SemanticApplicabilityGate | DONE | M13-06 | 73ac76d 已修 CI 断言文案 + **GQ-007 代码修复**（本次 v3）：`SemanticApplicabilityEvaluator.ResolveMetricAsync` 召回深度提至 50 + 全量前向匹配回退；新增语义层关键词确定性兜底——`SearchByKeywordAsync`（Keywords/Synonyms 精确匹配，非依赖向量）与 `SearchByKeywordSubstringAsync`（实体词前缀子串匹配，覆盖「入库单ID/入库单号」等形态）；向量解析未稳定（缺词法证据或竞争歧义）时以精确关键词命中为权威解析并视为非竞争，且**优先「入库数量」作为独立关键词（而非同义词）的列**、EntityCount 优先表主键（明细表扣分）。ground-truth：golden 跑 GQ-007 解析为 `wms_storage_receipt_info.quantity`(431) + `wms_storage_receipt.id`(5508 表头 PK)，v3 确定性命中一致。**根因**：CI 向量排序偏序——`入库数量` 同时命中 431(关键词)/282(同义词 5392) 致竞争歧义、且 `入库单数量`(EntityCount) 之实体 `wms_storage_receipt` 表头 PK 未被向量召回；v3 以确定性关键词/子串命中纠正。**验证**：待 CI 复跑 V2.6（注意 V2.6 此前另因「启动 Qdrant CI 容器」Docker 基础设施失败，属 CI 环境问题，非本项）。 |
