...
| BI-01 | CI/M13-06 | GQ-007 多指标 SemanticApplicabilityGate | DONE | M13-06 | 73ac76d 已修 CI 断言文案 + **GQ-007 代码修复**（本次）：`SemanticApplicabilityEvaluator.ResolveMetricAsync` 增加 Top-N（=5）前向匹配回退——当 #1 候选缺 Golden SemanticText 直接证据时，回退到 Top-5 内显式包含该词的候选（严格，不会误接受错误列）。**根因**：golden 跑（2026-08-29，18/18 绿）与 CI 代码/种子数据完全一致（Semantic.csv/column.csv 仅初始提交 222f3c6；评估器单提交 c9220b6），差异在运行环境——CI 向量排序把正确列（5541 `wms_storage_receipt_info.quantity` 关键词已含「入库数量」/5392 同义词含「入库数量」）排到 #1 之后，严格词法锚定因此 BLOCK。该回退纠正排序偏序，使 GQ-007 在 CI 与 golden 环境一致通过。**验证**：待 CI 复跑 V2.6（注意 V2.6 此前另因「启动 Qdrant CI 容器」Docker 基础设施失败，属 CI 环境问题，非本项）。 |
