...
| CI-02 | M13-05 | CI 执行全部 Unit Tests | ACTIVE | M13-05 | **开工中（2026-09-13）**：新增 `unit-tests` job 全量跑 `tests/SuperBuilder_AI.Tests`。此前该工程**不在 `SuperBuilder_AI.CI.slnf` 中**，其 1061 个 `[Fact]/[Theory]` 从未在 CI 编译或执行（改动破坏单测无人发现）。要点：测试工程"自带完整 restore+build"且**不**携带全局 `-p:TargetFrameworks`（否则污染 NuGet 资产关联 → xunit 适配器不进输出 → 0 用例假绿）；`Components` 在 `CI=true` 下只产 `net10.0`（规避 android/ios workload）；步骤内校验 `Total>0` 防假绿。待 CI 绿后置 DONE |
