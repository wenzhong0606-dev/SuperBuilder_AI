...
- [x] **P7.4 Theme 管理端点（CRUD + 指派）+ P7 总验收 ✅**（build 0 error；单测 147/147；`ThemeController` 租户作用域 CRUD + 指派租户默认 + 复制 + 蓝图 + `ThemeDslSerializer`；**Golden 18/18 PASS（限流窗口恢复后复跑 decision=PASS, 18/18, failedGates:None, 0 ERROR）**）
...
- [x] **P8.3 AppBuilderController 端点** ✅（build 0 error；单测 179/179；Golden 18/18 PASS；租户作用域 CRUD + 生成端点 + 编辑器蓝图，编排委托 IAppBuilderAgent 双路径零回归）
- [x] **P8.4 P8 总验收** ✅（build 0 error；单测 179/179；Golden 18/18 PASS；端到端应用生成冒烟：蓝图→结构化创建→读取回填一致→列表→跨租户隔离→清理；自然语言生成路径 Qwen 限流 502 best-effort 非阻断）
...
- [x] **P9.3 AgentController 端点 + 异常检测/原因分析链路** ✅（build 0 error；单测 231/231；Golden 18/18 PASS；`AgentController` 租户作用域 CRUD + 生成（LLM 路径失败 502）+ 工具目录 + 异常原因分析链路 6 步（销售额→同比→环比→区域→客户→产品→渠道）+ 编辑器蓝图；`AgentControllerTests` 17 项）
- [x] **P9.4 P9 总验收 + 工具选择测试 + 端到端冒烟 ✅**（build 0 error；单测 231/231；Golden 18/18 PASS；工具选择测试 ToolRegistryTests/AgentPlannerTests/AgentControllerTests 共 39 项；端到端 Agent 计划生成冒烟 `p9_e2e_smoke` 23/23 覆盖租户 CRUD/跨租户隔离/异常链/蓝图/LLM 路径/清理。P9 全阶段闭合）
...
- [x] **P10.5 Observability 中间件 + 架构依赖校验（反射等价 NetArchTest）+ P10 总验收 ✅**（build 0 error；单测 294/294；Golden 18/18；安全/审计基线测试）
...
5. **P11.2 旗舰页 ✅ 已完成**：`Ask.razor` 重写为多轮对话 + 端到端渲染（AI 解读 / KPI 卡 / Chart.js 图表 / 数据表 / SQL）+ **视图层多轮调整**（纯前端 DSL 变更、零后端调用、即时重渲染）+ **发布为应用**（序列化 App DSL v1.0 → `POST api/apps`，复用 P8 `BuildFromDslAsync`）。Chart.js 本地化（`chart.umd.min.js` v4.4.1）。**本轮增强（2026-08-31）**：新增统一错误治理（`ApiErrors` + `UnifiedExceptionMiddleware` + `AuthMiddleware` 401 统一 `ApiError`，零业务抛点改动、Golden 不受影响）+ Bootstrap 5.3.3 本地化 + 中国古风色系主题（宣纸/墨/黛蓝/朱砂/藤黄，覆盖 `--bs-*` + dark 变体 + 响应式）。三项目 build 0 error；Web Head 运行时 `/`·`/ask`·`/login` 与 RCL 静态资源（含 Bootstrap）均 HTTP 200。现有 API src 零改动，Golden 18/18 不受影响。
6. **P11.3 其余页面 + 组件库页 + 主题编辑器 ✅ 已完成（2026-08-31，页面部分）**：新建共享 `PageHead`（图标+标题+描述+右侧操作区）与 `TablePresenter`（任意 JSON 数组 → 友好表头/单元格/状态徽章）；`IApiClient`/`ApiClient` 新增松类型 `GetJsonAsync`（不抛异常，HTTP 非 2xx 与网络/解析错误一律经 err 返回，页面优雅降级）。重建 15 个页面：**数据类** Dashboards / Apps / BusinessModel / SemanticLabels / Agent（接后端只读 GET，统一加载·错误·空态 + 搜索/表格）；**平台类** DataSources（连接器网格+测试连接）/ ModelAccounts（模型卡片+BYO Key 绑定）/ Components（组件库展示，自包含）/ Themes（主题编辑器：调色板实时预览 + 明·暗预设切换）；**管理后台** Admin/{Tenants, Identity, Audit, Quota, Localization, Themes}。`app.css` 扩展 `page-head`/`stat-grid`/`stat-tile`/`panel`/`toolbar`/`badge`/`empty-state`/`field-grid`/`seg`/`row-list`/`swatch-grid` 等工具类；`NavMenu` 图标精灵补 `sb-ico-search`（现共 22 symbol，引用 18 个全部有定义）。三项目 build 0 error；Web 冒烟 **18 个页面全 200** + 静态资源 200；现有 API src 零改动，Golden 18/18 不受影响。**踩坑**：含 C# 字符串的事件处理器须用单引号定界（`@onclick='() => Toast("x")'`）；渲染名为 `code` 的变量须写 `@(code)`（否则被当作 `@code` 指令）；void 方法绑定须包 lambda。**遗留**：ask/refine（多轮语义调整）需新增只读端点，未实现。
7. **RCL 多目标 + Android/iOS 原生打通 ✅ 已完成（2026-08-31）**：RCL `SuperBuilder_AI.Components` 改多目标 `net10.0;net10.0-android;net10.0-ios`，MAUI 头改 `net10.0-android;net10.0-ios;net10.0-windows10.0.19041.0`，补齐 `Platforms/Android`（MainActivity/MainApplication/AndroidManifest）、`Platforms/iOS`（AppDelegate/Program）、`Resources/AppIcon`、`Resources/Splash`。**验证**：Android 真原生构建 0 error 并产出 `.apk` + `-Signed.apk`（约 16MB）；iOS 0 error，完成托管编译 + AOT + 原生静态库链接 + 多语言资源收集 + codesign 清单（最终 `.app` 打包/签名需配对 Mac 宿主，属平台限制）；Web Head 与 MAUI Windows 复验 0 error（零回归）。**⚠️ 关键坑**：RCL 不可引用 `Microsoft.AspNetCore.Components.WebView.Maui`——它会随 RCL 发布 `_framework/blazor.modules.json` 等宿主静态资源，与 MAUI 应用自身资源冲突，且 `StaticWebAsset Remove` 无法拦截（包资源在构建期注入）；RCL 统一只引 `Components.Web` + `Extensions.Http`。
8. **P11.5 优化轨道（性能/成本 + 安全/运维）✅ 起步（2026-08-31）**：①Ask 语义响应缓存——`src/Api/Caching`（`AskCacheOptions`/`IAskResponseCache`/`MemoryAskResponseCache`，基于 `IMemoryCache`，键=租户+归一化问题，仅缓存 `Success==true`，默认 TTL 60s/LRU 200），仅挂 `AskController`（可选构造参数，保持 2 参单测不破），支持 `?noCache=1` 与 `X-Cache` 头；命中跳过整条 BI 链路，Golden 走独立端点不受影响。②请求指标 + `GET /metrics`——`ObservabilityMiddleware` 注入 `RequestMetricsCollector`（可选参数），按路由聚合请求数/错误/P95/平均延迟（GUID/数字段归一化、限桶防基数爆炸），端点输出 `routes[]` + `askCache` 命中率。后端 build 0 error；单测 308/308；`/metrics` 实测采集、`/api/ask` 无令牌仍 401。③**鉴权收尾（P11.5.3，2026-08-31，已完成待提交）**：核心守卫 `AuthMiddleware`+`TokenService`(HMAC 无状态令牌) 早已在 P11.0 落并接进管道（计划 §9.3/第227行「无鉴权中间件」为过时记录，已修正）；本轮补「生产可用收口」——`appsettings.json` 配 `Auth:SigningKey`(强随机，消除 TokenService 回退 dev 默认串导致**可伪造 token**漏洞)；前端 `AuthStore`(RCL Scoped) 做 localStorage 持久化 + 启动经 `GET /api/auth/me` 自举校验(刷新不掉登录)；`ApiClient` 401→`AppState.NotifySessionExpired`→`MainLayout` 跳 `/login`(过期自动回收)。三端构建 0 error；单测 308/308；运行时实测 无token→401 / 坏token→401 / 合法token→200 / `/metrics`→200。Golden 走独立 `evaluation/golden-runtime` 端点隔离。
