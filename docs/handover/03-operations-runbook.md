...
> 受众：**[运维]** 客户 IT / SRE
> 所属：M14-07 交接包｜状态：**草稿**（服务目标待 PERF-01 定稿）
...
> ⚠️ **全部为占位，定稿前不得对外承诺。** 数值须由 PERF-01/M13-12 实测后回填，且**不得高于实测能力**（M14-07 验收判据 4）。

| 指标 | 目标值 | 依据 | 状态 |
|---|---|---|---|
| 可用性（月度） | 【待实测回填】 | PERF-01 | 未定 |
| Ask 接口 P95 延迟 | 【待实测回填】 | PERF-01 | 未定 |
| 实时查询接口 P95 延迟 | 【待实测回填】 | PERF-01 | 未定 |
| 请求失败率上限 | 【待实测回填】 | PERF-01 | 未定 |
| 并发承载 | 【待实测回填】 | PERF-01 | 未定 |
| 数据丢失窗口 **RPO** | 【待定稿】 | DR-01 手册 §3（建议值待平台 owner 裁定） | 未定 |
| 恢复时间 **RTO** | 【待定稿】 | 同上 | 未定 |
| 每请求成本上限 | 【待实测回填】 | PERF-01 | 未定 |

> 上述数值定稿后，**必须同步**更新：本表、`04-incident-and-support.md` §3（承诺口径）、`docs/ops/observability-troubleshooting.md` §3（生产级阈值）。
...
| 双实例支持状态 | 列为 CACHE-01 验证项 | `【待 OPEN「支持环境」确认单实例试点 / 双实例】` |
