...
/// Golden SemanticText 只与 Runtime SemanticText/语义别名比较；ColumnName 只承担物理绑定验证。
/// </summary>
public sealed class QueryPlanDimensionScoringService
{
    ...
    private static QueryPlanDimensionItemScore EvaluateDimension(GoldenDimensionExpectation expected, IReadOnlyList<QueryDimension> actual)
    {
        ...
        return new() { SemanticText = expected.SemanticText, RuntimeAlias = match.Alias, RuntimeColumn = match.ColumnName, RuntimeSemanticType = match.SemanticType, SemanticScore = semantic, BindingScore = binding, Score = score, Passed = passed, Reason = passed ? "Dimension matched and is physically bound." : binding < 1d ? "Dimension semantic match exists, but physical column binding is incomplete." : "Dimension semantic match failed." };
    }
