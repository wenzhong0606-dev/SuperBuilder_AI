...
public sealed class BusinessEntityGoldenFixtureController : ControllerBase
{
    ...
    public async Task<IActionResult> Provision(long tenantId, long dataSourceId, long metadataTableId, CancellationToken cancellationToken = default)
    {
        ...
        if (entity is null)
        {
            ...
            {
                TenantId = tenantId,
                BusinessKey = GoldenBusinessKey,
                Name = "GoldenCustomer",
                DisplayName = "Phase 3.1 Golden Customer",
                Description = "Runtime verification fixture; safe to remove after Phase 3.1 validation.",
                BusinessDomain = "Evaluation",
                SemanticText = "Golden customer business entity for Phase 3.1 runtime verification",
                Status = "Active"
            };
    ...
    private static void EnsureBinding<T>(ICollection<PhysicalBinding> bindings, long dataSourceId, long tableId, long columnId, string role) where T : class
    {
