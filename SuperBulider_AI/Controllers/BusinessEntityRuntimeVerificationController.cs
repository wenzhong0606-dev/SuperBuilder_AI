...
public sealed class BusinessEntityRuntimeVerificationController : ControllerBase
{
    ...
    {
        var entities = await _context.BusinessEntities.AsNoTracking()
        ...
        foreach (var s in dataSources) html.Append($"<tr><td>{s.TenantId}</td><td>{H(s.Name ?? "-")} (#{s.Id})</td><td>{H(s.DbType ?? "-")}</td><td>{s.Enabled}</td></tr>");
    ...
    [HttpPost("run")]
    [ApiExplorerSettings(IgnoreApi = true)]
    public async Task<IActionResult> Run([FromForm] long businessEntityId, [FromForm] long dataSourceId, CancellationToken cancellationToken = default)
    {
        ...
        if (entity is null)
            return Content(RenderError($"BusinessEntity not found. Received businessEntityId={businessEntityId}."), "text/html", Encoding.UTF8);
        ...
        if (source is null)
            return Content(RenderError($"Selected DataSource does not belong to the BusinessEntity tenant. Received dataSourceId={dataSourceId}, tenantId={entity.TenantId}."), "text/html", Encoding.UTF8);
    ...
    private static bool AllResolutionDataSourcesMatch(QueryPlanSemanticResolution r, long id) => r.Metrics.All(x => x.DataSourceId == id) && r.Dimensions.All(x => x.DataSourceId == id) && r.Filters.All(x => x.DataSourceId == id) && r.Tables.All(x => x.DataSourceId == id) && r.Orders.All(x => x.DataSourceId == id);
    private static IEnumerable<PhysicalBinding> AllBindings(BusinessEntity e) => e.Keys.SelectMany(x => x.PhysicalBindings).Concat(e.Attributes.SelectMany(x => x.PhysicalBindings)).Concat(e.Metrics.SelectMany(x => x.PhysicalBindings)).Concat(e.SourceRelationships.SelectMany(x => x.PhysicalBindings)).Concat(e.TargetRelationships.SelectMany(x => x.PhysicalBindings));
