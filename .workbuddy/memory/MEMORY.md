...
- 里程碑交付（截至 2026-09-08，git HEAD = M8-07）：
  ...
  - **M9-01 拆分 God ApiClient ✅（2026-09-08，994/994 测试全绿、四端 0 error）**：`ApiClient` 由 761 行单体重构为 `IApiClient` 向后兼容门面 + 7 个聚焦域客户端（Identity/Admin/BI/App/Dashboard/Agent/DataSource，各含 `I*ApiClient` 接口与 `ApiClientBase` 基类）；39 个既有调用点零改动。M9-02~15 仍为发布门禁未关闭。
