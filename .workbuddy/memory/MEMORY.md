...
> 唯一事实来源：`docs/Master_Development_Plan.md`（v2.4）+ `docs/milestones/Mn.md`。
...
## 测试基线
- 单测 `SuperBuilder_AI.Tests` **1140/1140 绿**（2026-09-12；基线随 M12 递增：1061→1071→1073→1075→1082→1091→1108→1119→1132(增量3-1)→**1140**(增量3-2)）。⚠️ `RateLimitMiddlewareTests.Expired_Windows_Are_Evicted...` 时间敏感 flaky（全量偶发失败、单跑 7/7 绿）。
...
- ✅ **M12 追加两项产品化增量（2026-09-12）**：**增量 3-1**（组织目录 重命名/启停/删除/列表分页，`8bca05a`，9 端点 + 14 i18n，全量 1132 绿，无新迁移）；**增量 3-2**（指标/维度编辑 + 计算口径 `Expression` + 数据类型 `DataType`，`80f34cd`，迁移 `M12_18`，全量 **1140/1140** 绿，开发库 45/45）。两者已回填 `M12.md` 交付记录。
- 🆕 **M13 / M14 立项（2026-09-12，文档-only，未开工）**：`docs/milestones/M13.md`（产品化收口与企业试点发布门禁，16 项 / 7 个 P0；G0→G1→G2 三段门槛）、`M14.md`（商业化交付/权益/经营闭环，8 项）。MDP 升至 v2.4。**承接关系**：M7-12→M13-07/08、M9-14→M13-10、M9-15→M13-04、M7-11+M12 交互验收→M13-09；M9-12 跳过、M10-01~03 保原编号。**用户决策（2026-09-12）：暂不启动 M13，先消缺口。**
- ⚠️ 本地 `HEAD == origin/master`（2026-09-12 = `f185abf`，已 `ls-remote` 独立确认）。**沙箱 push 凭据会过期**：报 `could not read Username for 'https://github.com': terminal prompts disabled` 时，用 `git -c credential.helper=manager push origin master` 绕过 `helper-selector`（后者仅交互可用）。
...
3. **新租户语言（部分修正，2026-09-12 复测）**：`TenantLanguageService.EnsureTenantLanguagesAsync`（:205）在**无显式配置**时继承平台全部启用语言 → 该路径（DemoData 安装）语言切换器可用。⚠️**但** `POST /api/tenant-management` 建租户时 `request.AvailableCultures ?? new[]{"zh-CN"}`（`TenantManagementController.cs:146`）→ **显式传单语言，永不触发全语言回退** ⇒ 经租户管理新建的租户默认只有 zh-CN，语言切换器（需 `AvailableCultures.Count>1`）不出现。真实缺口：创建 UI 需语言多选或默认继承平台全语言。
...
6. ✅ **已关闭（2026-09-12 回查源码）**：① `Agent.razor:163` 注释与实现均走真实 `POST api/agent/plan`，新建不再 Toast；② `DataSources.razor:207` 新建态「测试连接」实测调 `POST api/data-sources/test-connection`（连接串预测试端点已实现，方案 B），非 Toast。
...
- 历史缺陷：`M7_02_AppVersion` 曾含 39 个重复 CreateTable（已重写，仅建 `AppVersions`）；`AppPlans` 4 个发布列曾缺失（补丁 `20260910053000_M7_02_Fix_AppPlanPublishColumns`）。当前 **45 迁移**（增量 3-2 的 `20260911231209_M12_18_MetricDimensionExpression` 为末位，4 个 AddColumn）、差集 0。
- **开发库 `SuperBuilder_Platform` 已于 2026-09-12 同步到 45/45（pending 0 / drift 0）**；清单 `scripts/schema/schema-version.json` 同步刷新为 45（schemaVersion=`20260911231209_M12_18_MetricDimensionExpression`）。⚠️ 每次新增迁移必须重生成清单，否则校验报漂移。
