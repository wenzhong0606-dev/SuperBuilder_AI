...
## 测试基线
全量单测 `SuperBuilder_AI.Tests` = 1015/1015 绿、build 0 error（2026-09-08，含 M7-11 后）。Golden 契约 `Evaluation/Golden/query-plan-golden-v1.json` 18/18 未改。
...
- **实现进度(2026-09-08)**：C1/C2 后端已完成。
  ...
  - **待办：P0-D1/D2 验收。** P0-D2 E2E 用例已写(`AppRuntimeE2ETests.cs`+data-testid 钩子)，编译 0 错误，待 `SB_E2E_BASE_URL`+凭据+Chromium 执行；P0-D1 需平台管理员 demo 凭据（未入仓）。
  ...
  - **登录约束（E2E 关键）**：`platform` 租户被登录 UI 刻意排除（`AuthController.LoginOptions`/`TenantByCode` 两 GET 排除 `TenantCode=="platform"`），但 `POST /api/auth/login` 仅按数字 `TenantId` 校验、不排除 platform → `platform-admin`(TenantId=5) 可经 **`/admin/login` 页面**或 **API 登录** 进入（`/admin/tenants` 等由 `PlatformTenantView`/`PlatformAdminManage` 守卫；`NavMenuItems` 在持平台权限时显示）。E2E `LoginHelper.ApiLoginAsync(page,user,pwd,tenantId)` 已实现（fetch 登录→注入 `sb_auth_v1` 快照→进 /ask）；`AppRuntimeE2ETests` 管理员/读者均走此路径，`SB_E2E_TENANT=5`/`SB_E2E_READER_TENANT=1`（数字 Id）即正确。demo 限流 429 来自 `RateLimitMiddleware`（默认 LoginLimit=10/60s，localhost 同 IP 共用桶）→ 调大 `RateLimit:LoginLimit/GlobalLimit`+重启实例即解。
