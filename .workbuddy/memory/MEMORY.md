...
- **Stage 2 产品演进（P-track，用户10阶段路线）**：✅ P3 已完成，**P4 Multi-Tenant Platform Core ✅ 已全绿（P4.1/P4.2/P4.3/P4.4 均 ✅；build 0 error；跨租户单测 4/4；Golden 18/18 PASS ×P4.2/P4.3/P4.4 三轮）**。**P5 Multi-Language Runtime ✅ 已全绿（P5.1~P5.4 均 ✅；单测 65/65；Golden 18/18 PASS ×4 轮）**。**P6 Low-code BI Engine ✅ 已全绿（P6.1/P6.2/P6.3/P6.4 均 ✅；单测 117/117；Golden 18/18 PASS）**。**P7 Multi-Theme / Style Engine ✅ 已全绿（P7.1~P7.4 均 ✅；单测 147/147；Golden 18/18 PASS ×4 轮 P7.1/P7.2/P7.3/P7.4）**：P7.1 领域聚合+持久化+内置默认主题；P7.2 ThemeContext 接入 PlatformContext + 级联解析服务；P7.3 渲染引擎接入主题（ThemeRenderMapper + 逐组件 StyleSpec）；P7.4 Theme 管理端点（CRUD+指派+复制+蓝图）+ P7 总验收闭合。A3/A5 仍缓（用户指令）。
  ...
  - P3 Business Semantic Model / P4 Multi-Tenant Core / P5 Multi-Language / P6 Low-code BI / P7 Theme ✅(全绿) / P8 AI App Builder ✅(P8.1~P8.4 全绿) / P9 AI Agent / Copilot ✅(P9.1~P9.4 全绿, 单测 231/231, Golden 18/18 PASS ×3 轮) / P10 Enterprise SaaS 进行中(P10.1 ✅ Identity 基础设施：User/Role/Permission/UserRole/RolePermission + 迁移 20260830093125_P10_1_Identity + IIdentityService/IdentityService 幂等种子+确定性 RBAC + IdentityCatalog；单测 240/240；Golden 18/18 PASS ×1 轮 / P10.2 ✅ IdentityController 端点：api/identity 租户作用域用户/角色/权限 CRUD+角色指派撤销+权限解析+全局守卫；单测 256/256；Golden 18/18 PASS ×1 轮 / P10.3 ✅ AuditLog 审计日志：AuditLog 领域+持久化+IAuditLogService/AuditLogService 结构化审计(谁/什么/何时/结果)+租户作用域查询(确定性不调LLM)+AuditController(api/audit 查询/手动记录)+AuditMiddleware 自动请求审计(非阻塞静默)；迁移 20260830104059_P10_3_AuditLog；单测 268/268；Golden 18/18 PASS ×1 轮) / P10.4 ✅ Billing/Quota 配额：QuotaPolicy/QuotaUsage 领域+持久化+IQuotaService/QuotaService 幂等种子平台默认配额+租户覆盖优先回退+周期键(Total/Monthly/Daily)滚动归零+Check/Consume enforcement(确定性不调LLM)+QuotaController(api/quota 概览/单资源/校验/扣减)；迁移 20260830111544_P10_4_Quota；单测 285/285；Golden 18/18 PASS ×1 轮 / P10.5 ✅ Observability 中间件+架构依赖校验+P10 总验收：ObservabilityMiddleware(关联ID透传+请求/响应日志+耗时, 复用 AuditMiddleware 范式 非阻塞静默 不触碰响应体, Program 在 Audit 前注册) + ArchitectureTests(反射实现等价 NetArchTest, 校验 Domain/Ports/Services/Controllers 不反依赖外层; NuGet 镜像缺 NetArchTest 包故改反射实现) + P10AcceptanceTests(安全/审计基线: RBAC deny-by-default+租户隔离+审计记录查询往返+配额平台默认 enforcement); 单测 294/294；Golden 18/18 PASS ×1 轮)。**P10 整体闭合：P10.1~P10.5 全 ✅，Golden 18/18 全程保持；用户产品路线 10 阶段(P3~P10)全部完成**。A3/A5 仍缓（用户指令）。
  - **P11 前端（MAUI Blazor Hybrid + Blazor Web 共享 RCL）**：用户选定一套 UI 双端；本机 maui 工作负载齐备。**P11.0 后端前置 ✅**（api/ask + 鉴权中间件 + CORS + 限流 + /health；单测 308/308；Golden 18/18）。**P11.1 脚手架 ✅**（2026-08-31）。**P11.2 旗舰页 ✅**（2026-08-31 + 本轮增强）：`Ask.razor` 多轮对话+端到端渲染(AI解读/KPI卡/Chart.js图表/数据表/SQL) + **视图层多轮调整**(纯前端 DSL 变更零后端调用即时重渲染) + **发布为应用**(POST `api/apps` 复用 P8 `BuildFromDslAsync`)；Chart.js 本地化。**本轮新增统一错误治理**：后端 `src/Api/Errors/`（ErrorCodes/SuperBuilderException/ApiError）+ `src/Api/Middleware/UnifiedExceptionMiddleware.cs`（捕获未处理异常→结构化 `ApiError{code,message,traceId,details}` 友好 JSON + 关联ID 结构化日志，按异常类型/消息映射错误码，**零业务抛点改动、Golden 不受影响**；AuthMiddleware 401 也统一 ApiError）；前端 `ApiClient.ParseApiError` + `Ask.razor` 古风告警展示「错误码+追踪ID」。**本轮新增 Bootstrap 5.3.3 本地化**（`wwwroot/lib/bootstrap`）+ **中国古风色系** `app.css`（宣纸/墨/黛蓝/朱砂/藤黄/石青/黛绿，覆盖 `--bs-*` + dark 变体 + 响应式）；Login/Ask/NavMenu 套用 `btn`/`form-control`/`alert`。RCL `SuperBuilder_AI.Components` + Web Head `SuperBuilder_AI.Web`(**Blazor Server 经典模型**) + MAUI Head `SuperBuilder_AI.Maui`(**Blazor Hybrid**，Windows 目标 0 error)；三项目全 build 0 error、现有 API src 零改动、Golden 不受影响；纳入 `SuperBulider_AI.slnx`。注意 .NET 10 MAUI 需显式引 `Microsoft.Maui.Controls`+`Microsoft.AspNetCore.Components.WebView.Maui`(10.0.20)，`UseMauiBlazorWebView()` 扩展已移除仅用 `AddMauiBlazorWebView()`；RCL JS(`chart.js`)须显式 `<script>` 引入。阶段 P11.3~P11.5 待启动。**WMS 业务库 192.168.16.120:3306 现已可达**（2026-08-31 用户确认），但 `api/ask` 仍受产品语义/置信度闸门约束（部分问题 200 success:false 或 400 友好错误），需高置信+语义支持的问题才进 SQL 取数。设计文档 `docs/P11_Frontend_MAUI_Blazor_Plan.md`。
  - **P11.0 后端前置 ✅ 已完成（2026-08-31 验证+提交）**：全新增文件 + Program.cs 编辑，未碰 Golden 依赖。新增 `api/ask`(NL问数端点，需 DashboardView 权限) + `AuthMiddleware`(HMAC 无状态令牌，零 NuGet 依赖，匿名白名单保证 Golden 不受影响) + `RateLimitMiddleware`(120/分) + `AuthController`(api/auth 登录/me) + `TokenService` + CORS`P11Cors` + `/health`。build 0 error；单测 308/308（原 294 + 14）；**Golden 18/18 PASS**（decision=PASS, expectedOutcomePassed 18, failedGates=None）。解决核实硬缺口：①NL问数端点缺失 ②api/* 零鉴权(P10.2 仅权限 CRUD 无逐请求守卫)。下一步 P11.1 前端脚手架。
