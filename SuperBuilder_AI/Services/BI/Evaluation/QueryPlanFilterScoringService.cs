...
/// <summary>
/// Phase 2.6 C.13.2：Filter Evaluation。
/// SemanticText 与 Field 严格分离；Operator / Value 继续执行直接断言。
/// </summary>
public sealed class QueryPlanFilterScoringService
{
    public QueryPlanFilterEvaluationResult Evaluate(IReadOnlyList<GoldenFilterExpectation>? expected, IReadOnlyList<QueryFilter>? actual)
    {
        if (expected is null) return new() { Applicable = false, Passed = true, Score = 1d, Reason = "Filters are not asserted by this case." };
        if (expected.Count == 0)
        {
        ...
        if (actual is null || actual.Count == 0) return new() { Applicable = true, Passed = false, Score = 0d, Reason = "Expected filters are missing at runtime." };
        var items = expected.Select(x => EvaluateFilter(x, actual)).ToList();
        var score = items.Count == 0 ? 1d : items.Average(x => x.Score);
        return new() { Applicable = true, Passed = items.All(x => x.Passed), Score = score, Items = items, Reason = items.All(x => x.Passed) ? "All expected filters matched." : "One or more filter assertions failed." };
    }
    ...
    private static QueryPlanFilterItemScore EvaluateFilter(GoldenFilterExpectation expected, IReadOnlyList<QueryFilter> actual)
    {
        ...
        if (match is null) return new() { SemanticText = expected.SemanticText, SemanticScore = 0d, OperatorScore = 0d, ValueScore = 0d, BindingScore = 0d, Score = 0d, Passed = false, Reason = "No runtime filter matched the expected semantic filter." };
        var semantic = Equal(expected.SemanticText, match.SemanticText) ? 1d : 0d;
        ...
        return new() { SemanticText = expected.SemanticText, RuntimeField = match.Field, RuntimeOperator = match.Operator, RuntimeValue = match.Value, SemanticScore = semantic, OperatorScore = op, ValueScore = value, BindingScore = binding, Score = score, Passed = passed, Reason = failures.Count == 0 ? "Filter matched." : $"Filter mismatch: {string.Join(", ", failures)}." };
    }
