...
if (-not [string]::IsNullOrWhiteSpace($ConnectionString)) {
    ...
    $actual = @($raw | ForEach-Object { $_.ToString().Trim() } | Where-Object { $_ -match '^[0-9]{14}_' })
    ...
    $actual = @($raw | ForEach-Object { $_.ToString().Trim() } | Where-Object { $_ -match '^[0-9]{14}_[A-Za-z0-9_]+' } | ForEach-Object {
        ($_ -replace '\s.*$', '')
    })
